...
public class OpenApi30Config {
	...
	public OpenApi30Config(@Value("GamePlace") String moduleName, @Value("0.0.1-SNAPSHOT") String apiVersion) {
	...
	@Bean
	 public OpenAPI customOpenAPI() {
		 ...
		 .components(new Components().addSecuritySchemes(securitySchemeName, new SecurityScheme().name(securitySchemeName)
				 .type(SecurityScheme.Type.HTTP).scheme("bearer").bearerFormat("JWT"))).info(new Info().title(apiTitle).version(apiVersion));
	 }
